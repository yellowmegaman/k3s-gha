...
docker run -d --name=k3s --privileged --tmpfs /run --tmpfs /var/run -p 6443:6443 -p 80:80 -v /github/workspace/k3s.yaml:/etc/rancher/k3s/k3s.yaml --mount 'type=bind,src=/github/workspace/registries.yaml,dst=/etc/rancher/k3s/registries.yaml' rancher/k3s:$INPUT_K3S_TAG server
